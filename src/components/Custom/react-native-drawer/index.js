...
export default class Drawer extends Component {
    ...
    switch (this.props.type) {
      case 'overlay':
        drawerProps[this.props.side] =
          -this.state.viewport.width + this._offsetOpen + this._left;
  ...
  shouldOpenDrawer(dx) {
    let hasActiveHeading =
      this._open ^ (dx > 0) ^ (this.props.side === getRtlSide('right'));
  ...
  onPanResponderMove = (e, gestureState) => {
    ...
    if (
      this._open ^
      (gestureState.dx < 0) ^
      (this.props.side === getRtlSide('right'))
    )
      return false;
  ...
  processMoveShouldSet = (e, gestureState) => {
    ...
    let swipeInCloseDirection =
      this.props.side === getRtlSide('left') ? swipeToLeft : swipeToRight;
    if (
      swipeUpDown ||
  ...
  testPanResponderMask = (e, gestureState) => {
    ...
    let deltaOpen =
      this.props.side === getRtlSide('left')
        ? this.state.viewport.width - x0
        : x0;
    let deltaClose =
      this.props.side === getRtlSide('left')
        ? x0
